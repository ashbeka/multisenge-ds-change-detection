...
class SMBase(BaseEstimator, ClassifierMixin):
    ...
    def __init__(
        self, n_subdims, normalize=False, faster_mode=False, test_n_subdims=None
    ):
        """
        Parameters
        ----------
    ...
    def predict(self, X):
        ...
        assert (
            proba.min() > 0.0 - salt
        ), "some probabilities are smaller than 0! min value is {}".format(proba.min())
        assert (
            proba.max() < 1.0 + salt
        ), "some probabilities are bigger than 1! max value is {}".format(proba.max())
...
class KernelSMBase(SMBase):
    ...
    def __init__(
        ...
        """
        super(KernelSMBase, self).__init__(
            n_subdims, normalize, faster_mode, test_n_subdims
        )
        self.sigma = sigma
